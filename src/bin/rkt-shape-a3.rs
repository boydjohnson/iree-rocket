//! NPU diagnostic: rkt-shape-a.rs and rkt-shape-a2.rs both showed only
//! output byte 0 getting written (128), bytes 1-15 staying zero. New
//! theory before assuming a register-content bug: `regcmd.rs`'s own
//! `surfaces_per_row` calculation (`output_width * output_height * 2`,
//! unconditional `*2` even for non-depthwise) hints that each output
//! pixel occupies *2 atomics* worth of space in the output buffer, not 1
//! byte -- because `output_channels=1` gets padded to
//! `task_output_channels=32` (`align(max(1,32),32)`), and CNA/DPU operate
//! in 16-channel "atomics" (`FEATURE_ATOMIC_SIZE=16`), so 32 padded
//! channels needs 2 atomics = 32 bytes per pixel, not 1. If that's right,
//! reading only the first 16 contiguous bytes (like rkt-shape-a.rs did)
//! only ever looks *inside pixel [0,0]'s own padding*, and never reaches
//! pixel [0,1] at all (which would start at byte offset 32, not byte 1).
//!
//! This variant dumps the first 256 bytes of the output buffer instead of
//! just 16, to check for nonzero data at byte offsets 32/64/96/... instead
//! of assuming a flat 1-byte-per-pixel layout.
...
fn main() {
    ...
    println!(
        "--- NPU Diagnostic: shape A3, same known-good 4x4x1/1x1-kernel shape, 256-byte output readback (testing per-pixel stride theory) ---"
    );
    ...
    unsafe {
        ...
        match rocket_prep_bo(fd, &prep) {
            Ok(_) => {
                let out = std::slice::from_raw_parts(buf_c.host_ptr, 256);
                println!("Done! First 256 output bytes, 32 per row:");
                for (row, chunk) in out.chunks(32).enumerate() {
                    println!("  [{:4}..{:4}) {:?}", row * 32, row * 32 + 32, chunk);
                }
                let nonzero_offsets: Vec<usize> = out
                    .iter()
                    .enumerate()
                    .filter(|(_, b)| **b != 0)
                    .map(|(i, _)| i)
                    .collect();
                println!("Nonzero byte offsets: {:?}", nonzero_offsets);
            }
            Err(e) => {
