...
//
// The scalar operand feeds `EW_OP_VALUE_0` and only `add_scalar` reads it.
// `build_unary_regcmd` asserts the same rule, so a stray operand must be a
// compile error rather than a panic inside the builder on a board.

// CHECK: 'operand' is only meaningful for the add_scalar element-wise op
